...
public interface ItemDao {
    ...
    @Query("SELECT i.* FROM Item i, ItemMap im WHERE i.status = 1 AND i.id = im.itemId AND im.mapKey = :value ORDER BY im.sorting asc")
    LiveData<List<Item>> getItemByKey (String value);

    @Query("SELECT i.* FROM Item i, ItemMap im WHERE i.status = 1 AND i.id = im.itemId AND im.mapKey = :value ORDER BY im.sorting asc limit:limit")
    ...
    @Query("SELECT isFavourited FROM Item WHERE status = 1 AND id =:itemId")
    public abstract String selectFavouriteById(String itemId);

    @Query("UPDATE Item SET isFavourited =:is_favourited WHERE status = 1 AND id =:itemId")
    ...
    @Query("SELECT prd.* FROM Item prd, ItemFavourite fp WHERE prd.status = 1 AND prd.id = fp.itemId order by fp.sorting ")
    ...
    @Query("SELECT item.* FROM Item item, ItemFromFollower itemFromFollower WHERE item.status = 1 AND item.id = itemFromFollower.itemId order by itemFromFollower.sorting ")
    LiveData<List<Item>> getAllItemFromFollower();

    //item detail

    @Query("SELECT * FROM Item WHERE status = 1 AND id =:itemId ORDER BY addedDate DESC")
